...
$this->getServiceLocator()->get('logService')->log(LogService::LEVEL_INFO, 'rex_cu_re_no', 'Customer retrieval not implemented yet.', array());return 0;$retailExpressData = 0;
        ...
        $message = 'Retrieved '.count($retailExpressDataById).' customers in '.$seconds.'s up to '
            .strftime('%H:%M:%S, %d/%m', $this->retrieveTimestamp).' via '.$api.' api.';
        $logData = array('type'=>'customer', 'amount created/updated'=>count($retailExpressData), 'period [s]'=>$seconds);
        if (count($retailExpressData) > 0) {
            $logData['per entity [s]'] = round($seconds / count($retailExpressData), 3);
        }
        $this->getServiceLocator()->get('logService')->log(LogService::LEVEL_INFO, 'rex_cu_re_no', $message, $logData);
